...
public class QuanLySanPhamJPanel extends javax.swing.JPanel {
    ...
    private void initComponents() {
        ...
        tblSanPhansd.setModel(new javax.swing.table.DefaultTableModel(
            ...
            new String [] {
                "Tên sản phẩm", "Loại sản phẩm", "Giá", "Đơn vị", "Trạng thái"
            }
        ));
        ...
        add(tabs);
        tabs.setBounds(30, 100, 1220, 349);
    ...
    private void btnStartActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnStartActionPerformed
        ...
        if (tabs.getSelectedIndex() == 0) {
            ...
            rows = 0;
            tblSanPHamksd.setRowSelectionInterval(rows, rows);
            editksd();
        }
    ...
    private void btnnextActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnnextActionPerformed
        ...
        if (tabs.getSelectedIndex() == 0) {
            ...
            if (rows < tblSanPHamksd.getRowCount() - 1) {
                rows++;
                tblSanPHamksd.setRowSelectionInterval(row, row);
                editksd();
            }
    ...
    private void btnLastActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnLastActionPerformed
        ...
        if (tabs.getSelectedIndex() == 0) {
            ...
            rows = tblSanPHamksd.getRowCount() - 1;
            tblSanPHamksd.setRowSelectionInterval(row, row);
            editksd();
    ...
    private void tblSanPHamksdMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_tblSanPHamksdMouseClicked
        // TODO add your handling code here:
        try {
            rows = tblSanPHamksd.getSelectedRow();
...
private void init() {
    ...
    private void filltotableSP() {
        ...
        try {
            ...
            for (SanPham x : list) {
                model.addRow(new Object[]{x.getTen_sp(), daolsp.selectNameByID(x.getId_loaiSP()),
    ...
    private void filltotableSPKD() {
        ...
        try {
            ...
            for (SanPham x : list) {
                modelspkd.addRow(new Object[]{x.getTen_sp(), daolsp.selectNameByID(x.getId_loaiSP()),
                    x.getGia_sp(), daodv.selectNameByID(x.getId_donviSP()),
                    x.isTrangthai() ? "sử dụng" : "không sử dụng"});
            }
        } catch (Exception e) {
    ...
    private void chooseImages() {
        JFileChooser file = new JFileChooser("C:\\Users\\top1z\\Downloads\\MEmu Download");
    ...
    private void setForm(SanPham sp) {
        String douong = tblSanPhansd.getValueAt(row, 1).toString();
        for (int i = 0; i < listlsp.size(); i++) {
            if (douong.equals(listlsp.get(i).getTenLoai())) {
                cboLoaidouong.setSelectedIndex(i);
                System.out.println(listlsp.get(i).getTenLoai());
            }
        }
        String donvidouong = tblSanPhansd.getValueAt(row, 3).toString();
...
//        String douong = tblSanPhansd.getValueAt(row, 1).toString();
        String douongksd = tblSanPHamksd.getValueAt(rows, 1).toString();
        for (int i = 0; i < listlsp.size(); i++) {
            if (douongksd.equals(listlsp.get(i).getTenLoai())) {
                cboLoaidouong.setSelectedIndex(i);
                System.out.println(listlsp.get(i).getTenLoai());
            }
        }
        String donvidouong = tblSanPHamksd.getValueAt(rows, 3).toString();
        for (int i = 0; i < listdv.size(); i++) {
            if (donvidouong.equals(listdv.get(i).getTenDonvi())) {
                cboDonvi.setSelectedIndex(i);
            }
...
//            lblHinh.setIcon(new ImageIcon("unnamed.png"));
    ...
    private void edit() {
        String tensp = tblSanPhansd.getValueAt(row, 0).toString();
        SanPham sp = DAOSP.selectTen(tensp);
        setForm(sp);
        updateStatus();
    }

    private void editksd() {
        String maspksd = tblSanPHamksd.getValueAt(rows, 0).toString();
        SanPham sp = DAOSP.selectTen(maspksd);
        System.out.println(DAOSP.selectTen(maspksd));
        setFormSP(sp);
        updateStatus2();
...
//            e.printStackTrace();S
    ...
    private void updateStatus2() {
        boolean edit = (rows >= 0);
        boolean fist = (rows == 0);
        boolean last = (rows == tblSanPHamksd.getRowCount() - 1);
        ////
